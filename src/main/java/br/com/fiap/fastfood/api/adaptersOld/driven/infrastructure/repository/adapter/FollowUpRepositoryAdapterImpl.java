...
import br.com.fiap.fastfood.api.adaptersOld.driven.infrastructure.entity.followup.FollowUpEntity;
import br.com.fiap.fastfood.api.adaptersOld.driven.infrastructure.mapper.FollowUpMapperInfra;
import br.com.fiap.fastfood.api.adaptersOld.driven.infrastructure.repository.follow.up.FollowUpRepository;
...
public class FollowUpRepositoryAdapterImpl implements FollowUpRepositoryPort {
  ...
  public List<FollowUpDTO> findAllByState(FollowUpStateEnum state) {
    return repository.findAllByState(
        br.com.fiap.fastfood.api.adaptersOld.driven.infrastructure.entity.followup.FollowUpStateEnum.valueOf(state.name())).stream().map(mapper::toDTO).collect(
        Collectors.toList());
  }
